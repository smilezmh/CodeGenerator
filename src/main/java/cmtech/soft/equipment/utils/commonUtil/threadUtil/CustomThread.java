...
public class CustomThread extends Thread {
    private Deque<Runnable> cacheTasks = new LinkedList<>();// 缓存的任务
    ...
    private Runnable getTask() {
        ...
        try {
            lock.lock();
            if (cacheTasks != null && cacheTasks.size() > 0) {
    ...
    private void removeFinishTaskFromCache(Runnable finishTask) {
        try {
            ...
            if (cacheTasks != null && cacheTasks.size() > 0 && cacheTasks.contains(finishTask)) {
